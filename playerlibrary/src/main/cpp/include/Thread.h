...
#ifndef DAFUPLAYER_THREAD_H
#define DAFUPLAYER_THREAD_H
...
/**
 * Thread can use a custom Runable, but must delete Runnable constructor yourself
 */
...
inline void Thread::start() {
 ...
 inline void* Thread::threadEntry(void *arg) {
     ...
     if (thread != NULL) {
         ...
         thread->mCondition.signal();//fixme 为什么这里发信号
 ...
 inline int Thread::schedPriority(ThreadPriority priority) {
     ...
     if(priority == Priority_Low){
         ...
         int min_priority = sched_get_priority_max(policy);
     } else {
