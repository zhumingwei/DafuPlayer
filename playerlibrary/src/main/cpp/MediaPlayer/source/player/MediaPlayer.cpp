...
int MediaPlayer::readPackets() {
    ...
    for (;;) {
        ...
        if (playerState ->pauseRequest &&
        (!strcmp(pFormatCtx->iformat->name, "rtsp") || (pFormatCtx->pb && !strncmp(url, "mmsh:", 5)))
        ){
                av_usleep(10 * 1000);
                continue;
            }
        ...
        if (playerState->seekRequest) {
            ...
            if (ret < 0) {
                ...
                if (!playerState->pauseRequest
                    && (!audioDecoder || audioDecoder->getPacketSize() == 0)
                    && (!videoDecoder ||
                        (videoDecoder->getPacketSize() == 0 && videoDecoder->getFrameSize() == 0))
                        ) {
                    if (playerState->loop) {
                        seekTo(playerState->startTime != AV_NOPTS_VALUE ? playerState->startTime
                                                                        : 0);
            ...
            playInRange = playerState->duration == AV_NOPTS_VALUE
                          ...
                          - (double) (playerState->startTime != AV_NOPTS_VALUE
                                      ? playerState->startTime : 0) / 1000000
                          <= ((double) playerState->duration / 1000000);
            if (playInRange && audioDecoder &&
                pkt->stream_index == audioDecoder->getStreamIndex()) {
                audioDecoder->pushPacket(pkt);
            } else if (playInRange && videoDecoder &&
...
int MediaPlayer::prepareDecoder(int streamIndex) {
    ...
    do {
        ...
        if (avctx->codec_type == AVMEDIA_TYPE_VIDEO || avctx->codec_type == AVMEDIA_TYPE_AUDIO) {
            av_dict_set(&opts, "refcounted frames", "1", 0);
        }
        ...
        switch (avctx->codec_type) {
            ...
            case AVMEDIA_TYPE_VIDEO: {
                videoDecoder = new VideoDecoder(
                        pFormatCtx, avctx, pFormatCtx->streams[streamIndex],
                        streamIndex, playerState
                );
                attachmentRequest = 1;
                break;
    ...
    if (ret < 0) {
        if (playerState->messageQueue) {
            ...
            playerState->messageQueue->postMessage(
                    MSG_ERROR, 0, 0, (void *) errorMsg, sizeof(errorMsg) / errorMsg[0]
            );
        }
...
int MediaPlayer::openAudioDevice(int64_t wanted_channel_layout, int wanted_nb_channels,
    ...
    const int next_nb_channnels[] = {0, 0, 1, 6, 2, 6, 4, 6};
    ...
    wanted_spec.freq = wanted_nb_channels;
    ...
    while (audioDevice->open(&wanted_spec, &spec) < 0) {
        ...
        wanted_spec.channels = next_nb_channnels[FFMIN(7, wanted_spec.channels)];
    ...
    if (spec.channels != wanted_spec.channels) {
        ...
        if (!wanted_channel_layout) {
            av_log(NULL, AV_LOG_ERROR, "channel cound %d is not supported!\n", spec.channels);
            return -1;
        }
    }
...
void MediaPlayer::pcmQueueCallback(uint8_t *stream, int len) {
    ...
    if (playerState->messageQueue && playerState->syncType != AV_SYNC_VIDEO) {
        playerState->messageQueue->postMessage(MSG_CURRENT_POSITION, getCurrentPosition(),
