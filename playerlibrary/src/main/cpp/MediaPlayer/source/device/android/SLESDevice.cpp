...
int SLESDevice::open(const AudioDeviceSpec *desired, AudioDeviceSpec *obtained) {
    ...
    SLDataLocator_OutputMix outputMix = {
            SL_DATALOCATOR_OUTPUTMIX,slOutputMixObject
    };
    ...
    SLDataFormat_PCM format_pcm = {
            ...
            getSlSampleRate(desired->freq), //采样率
    ...
    result = (*slEngine) -> CreateAudioPlayer(
            slEngine, &slPlayerObject, &slDataSource,
            &audioSink, 3, ids, req
            );
    if (result != SL_RESULT_SUCCESS){
        ALOGE("%s: slEngine->CreateAudioPlayer() fialed", __func__);
        return -1;
    }
    ...
    result = (*slPlayerObject)->GetInterface(
            slPlayerObject, SL_IID_VOLUME, &slVolumeItf
            );
    if (result != SL_RESULT_SUCCESS){
    ...
    //创建缓冲区
    buffer = (u_int8_t *) malloc(buffer_capacity);
    if (!buffer){
    ...
    for (int i = 0; i < OPENSLES_BUFFERS; ++i) {
        result = (*slBufferQueueItf)->Enqueue(
                slBufferQueueItf,
