...
void Metadata::setCodec(AVFormatContext *pFormatCtx, int streamIndex) {
    const char *codec_type = av_get_media_type_string(
            pFormatCtx->streams[streamIndex]->codec->codec_type);
    if (!codec_type) {
        return;
    }
...
void
Metadata::setRotation(AVFormatContext *pFormatCtx, AVStream *audioStream, AVStream *videoStream) {
    if (!extractMetadata(pFormatCtx, audioStream, videoStream, ROTATE) && videoStream &&
        videoStream->metadata) {
        AVDictionaryEntry *entry = av_dict_get(videoStream->metadata, ROTATE, NULL,
                                               AV_DICT_MATCH_CASE);
...
void
Metadata::setFrameRate(AVFormatContext *pFormatCtx, AVStream *audioStream, AVStream *videoStream) {
    char value[30] = "0";
...
const char *
Metadata::extractMetadata(AVFormatContext *pFormatCtx, AVStream *audioStream, AVStream *videoStream,
                          const char *key) {
    char *value = NULL;
    if (!pFormatCtx) {
        return value;
    ...
    if (key) {
        if (av_dict_get(pFormatCtx->metadata, key, NULL, AV_DICT_MATCH_CASE)) {
            value = av_dict_get(pFormatCtx->metadata, key, NULL, AV_DICT_MATCH_CASE)->value;
                   ...
                   av_dict_get(audioStream->metadata, key, NULL, AV_DICT_MATCH_CASE)) {
            value = av_dict_get(audioStream->metadata, key, NULL, AV_DICT_MATCH_CASE)->value;
        } else if (videoStream &&
                   av_dict_get(videoStream->metadata, key, NULL, AV_DICT_MATCH_CASE)) {
...
Metadata::extractMetadata(AVFormatContext *pFormatCtx, AVStream *audioStream, AVStream *videoStream,
    ...
    if (strcmp(key, CHAPTER_START) == 0) {
        ...
        value = static_cast<char *>(malloc(strlen(time)));//fixme static_cast ，以及malloc用法
        sprintf(value, "%s", time);
