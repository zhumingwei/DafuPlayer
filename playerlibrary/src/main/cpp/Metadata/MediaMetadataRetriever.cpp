...
int MediaMetadataRetriever::getEmbededPicture(AVPacket *pkt) {
    Mutex::Autolock lock(mLock);
 ...
 */
int
MediaMetadataRetriever::setDataSource(MetadataState **ps, const char *path, const char *headers) {
 ...
 */
const char *
MediaMetadataRetriever::extractMetadata(MetadataState **ps, const char *key, int chapter) {
    char *value = NULL;
    MetadataState *state = *ps;
        ...
        if (state->pFormatCtx->streams[i]->disposition & AV_DISPOSITION_ATTACHED_PIC) {
            ...
            if (pkt->stream_index == state->videoStreamIndex) {
                ...
                if (!formatSupport(codec_id, pix_fmt)) {
                    ...
                    //解码视频帧
                    if (avcodec_decode_video2(state->videoStream->codec, frame, &got_frame, pkt) <=
                        0) {
                        break;
                    }
...
int MediaMetadataRetriever::getFrame(MetadataState **ps, int64_t timeUs, AVPacket *pkt, int width,
    ...
    if (timeUs > -1) {
        ...
        int64_t seek_time = av_rescale_q(timeUs, AV_TIME_BASE_Q,
                                         state->pFormatCtx->streams[stream_index]->time_base);
...
int MediaMetadataRetriever::setDataSource(MetadataState **ps, const char *path) {
    ...
    //查找媒体流
    for (int i = 0; i < state->pFormatCtx->nb_streams; ++i) {
        if (state->pFormatCtx->streams[i]->codec->codec_type == AVMEDIA_TYPE_VIDEO &&
            videoIndex < 0) {
            videoIndex = i;
        }
        if (state->pFormatCtx->streams[i]->codec->codec_type == AVMEDIA_TYPE_AUDIO &&
            audioIndex < 0) {
            audioIndex = i;
        }
        //设置解码信息
    ...
    *ps = state; //fixme 原先就是这里有必要赋值吗，不设置大概率加载不出来视频
    return 0;
}
...
int MediaMetadataRetriever::initScaleContext(MetadataState *s, AVCodecContext *pCodecCtx, int width,
    ...
    if (!s->pScaleCodecContext) {
        ALOGE("avcodec_aloc_context3 failed\n");
        return -1;
    }
    ...
    s->pScaleSwsContext = sws_getContext(
...
int MediaMetadataRetriever::openStream(MetadataState *s, int streamIndex) {
    ...
    switch (codecCtx->codec_type) {
        ...
        case AVMEDIA_TYPE_VIDEO: {
            ...
            if (!pCodec) {
                ALOGE("avcodec_find_decodder() failed to find encoder\n");
                return -1;
            }
                ...
                *got_frame = 0;
                ...
                if (*got_frame) {
                    if (desired_frame_number == -1 ||
                        (frame->pts >= desired_frame_number)
                            ) {
                        if (pkt->data) {
                            av_packet_unref(pkt);
                        }
