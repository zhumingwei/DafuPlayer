...
static void
CainMediaMetadataRetriever_setDataSourceAndHeaders(JNIEnv *env, jobject thiz, jstring path_,
    ...
    if (!path_) {
        throwException(env, "java/lang/IllegalArgumentException", "Null path");
        return;
    }
    ...
    char *headers = NULL;
    if (keys && values != NULL) {
        int keysCount = env->GetArrayLength(values);
        int valuesCount = env->GetArrayLength(values);
        if (keysCount != valuesCount) {
            ALOGE("keys and values arrays have different length");
            throwException(env, "java/lang/IllegalArgumentException", "error kv");
            return;
        }
        int i = 0;
...
CainMediaMetadataRetriever_getFrameAtTime(JNIEnv *env, jobject thiz, jlong timeUs, jint option) {
    MediaMetadataRetriever *retriever = getRetriever(env, thiz);
    if (retriever == NULL) {
        throwException(env, "java/lang/IllegalStateException", "No retriever");
        return NULL;
    }
    AVPacket packet;
...
CainMediaMetadataRetriever_getEmbeddedPicture(JNIEnv *env, jobject thiz, jint pictureType) {
    ...
    av_init_packet(&packet);
    jbyteArray array = NULL;
    if (retriever->getEmbededPicture(&packet) == 0) {
        int size = packet.size;
        uint8_t *data = packet.data;
        array = env->NewByteArray(size);
        jbyte *bytes = env->GetByteArrayElements(array, NULL);
...
CainMediaMetadataRetriever_extractMetadataFromChapter(JNIEnv *env, jobject thiz, jstring key_,
    ...
    if (!key_) {
        throwException(env, "java/lang/IllegalStateException", "Null pointer");
        return NULL;
    }
...
static void
CainMediaMetadataRetriever_native_init(JNIEnv *env, jclass clazz) {
