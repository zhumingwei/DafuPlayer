...
void
CainMediaPlayer_setDataSourceFD(JNIEnv *env, jobject thiz, jobject fileDescriptor, jlong offset,
                                jlong length) {
    CainMediaPlayer *mp = getMediaPlayer(env, thiz);
    if (mp == NULL) {
        jniThrowException(env, "java/lang/IllegalStateException");
...
void CainMediaPlayer_setup(JNIEnv *env, jobject thiz, jobject mediaplayer_this) {
    ...
    // Stow out new c++ MediaPlayer in an opaque field in the Java object
...
jlong CainMediaPlayer_getDuration(JNIEnv *env, jobject thiz) {
    CainMediaPlayer *mp = getMediaPlayer(env, thiz);
    if (mp == NULL) {
        jniThrowException(env, "java/lagn/IllegalStateException");
        return 0L;
    }
...
jboolean CainMediaPlayer_isPlaying(JNIEnv *env, jobject thiz) {
    ...
    return (mp->isPlaying() ? JNI_TRUE : JNI_FALSE);
}

jboolean CainMediaPlayer_getRotate(JNIEnv *env, jobject thiz){
    CainMediaPlayer *mp = getMediaPlayer(env, thiz);
    if (mp == NULL) {
        jniThrowException(env, "java/lang/IllegalStateException");
        return 0;
