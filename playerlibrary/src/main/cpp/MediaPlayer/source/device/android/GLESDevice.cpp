...
GLESDevice::GLESDevice() {
    ...
    nodeList->addNode(new DisplayRenderNode()); //限时渲染结点
...
void GLESDevice::onInitTexture(int width, int height, TextureFormat format, BlendMode blendMode,
    ...
    if (eglSurface == EGL_NO_SURFACE && mWindow != NULL) {
        ...
        // 处于SurfaceDestoryed状态， 释放EGLSurface
        if (!mHasSurface) {
            terminate(false);
        }
    }
    ...
    // mVideoTexture->width fixme width 不需要吗
    ...
    if (filterChange) {
        nodeList->changeFilter(filterInfo.type,
                               FilterManager::getInstance()->getFilter(&filterInfo)
        );
        filterChange = false;
    }
    nodeList->init();
...
int GLESDevice::onUpdateYUV(uint8_t *yData, int yPitch, uint8_t *uData, int uPitch, uint8_t *vData,
    ...
    // 设置像素实际的宽度，即linesize的值 fixme 什么意思
