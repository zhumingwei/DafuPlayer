...
#ifndef DAFUPLAYER_PLAYERSTATE_H
#define DAFUPLAYER_PLAYERSTATE_H
...
public:
    Mutex mMutex;                   //操作互斥锁，主要给seek操作，音视频解码以及清空解码上下文缓冲使用，不加锁会导致ffmpeg内部崩溃现象
    AVDictionary *sws_dict;         //视频转码option参数
    AVDictionary *swr_opts;         //音视频重采样option参数
    ...
    int mute;                       //静音播放
    int frameDrop;                  //舍帧操作
    int reorderVideoPts;            //视频帧重拍pts
};


#endif //DAFUPLAYER_PLAYERSTATE_H
