...
void MediaSync::refreshVideo(double *remaining_time) {
    ...
    for (;;) {
        ...
        if (videoDecoder->getFrameSize() > 0) {
            ...
            // 获取当前时间
            time = av_gettime_relative() / 100000.0;
            ...
            //更新帧计时器
            frameTimer += delay;
            //帧计时器落后当前时间超过了阈值， 则用当前的时间作为帧计时器的时间
            ...
            mMutex.unlock();
            // 如果队列中还剩余超过一帧的数据时，需要难道下一帧，然后计算间隔，并判断是否需要进行舍帧操作
    ...
    if (playerState->messageQueue && playerState->syncType == AV_SYNC_VIDEO) {
        ...
        playerState->messageQueue->postMessage(
                MSG_CURRENT_POSITION, pos, playerState->videoDuration
        );
    }
...
void MediaSync::checkExternalClockSpeed() {
    if (videoDecoder && videoDecoder->getPacketSize() <= EXTERNAL_CLOCK_MIN_FRAMES
        ...
        extClock->setSpeed(
                FFMAX(EXTERNAL_CLOCK_SPEED_MIN, extClock->getSpeed() - EXTERNAL_CLOCK_MIN_FRAMES));
