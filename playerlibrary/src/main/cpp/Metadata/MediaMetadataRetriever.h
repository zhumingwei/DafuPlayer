...
#ifndef DAFUPLAYER_MEDIAMETADATARETRIEVER_H
#define DAFUPLAYER_MEDIAMETADATARETRIEVER_H
...
public:
    ...
    // 提取专辑/封面
    int getEmbededPicture(AVPacket *pkt);
...
private:
    ...
    //解码视频帧
    void decodeFrame(MetadataState *state, AVPacket *pkt, int *got_frame, int64_t desired_frame_number,
                int width, int height);

    //转码图片
    void encodeImage(MetadataState *state, AVCodecContext *pCodecCtx, AVFrame *pFrame,
                     AVPacket *pakcet, int *got_packet, int width, int height);
};

#endif //DAFUPLAYER_MEDIAMETADATARETRIEVER_H
